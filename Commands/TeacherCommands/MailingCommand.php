...
class MailingCommand extends TeacherCommand
{
    protected $name = 'mailing';

    protected $description = 'Розсилка повідомлення до всіх учнів';

    protected $usage = '/malling';
        ...
        switch ($state) {
            case 0:
                ...
                if ($text === '' || (!in_array($text, $groups, true))) {
                    ...
                    $data['text'] = 'Оберіть групу для розсилки повідомлення:';

                    if ($text === '' || $text === 'Розсилка повідомлення') {
            ...
            case 1:
                if ($text === '') {
                    ...
                    $data['text'] = 'Напишіть повідомлення для группи:' . $notes['group'];

                    $result = Request::sendMessage($data);
                    break;
                }

                $notes['group_message'] = "Вчитель: " . $teacher_user->first_name . " " . $teacher_user->second_name . " " . $teacher_user->middle_name . " розислав повідомлення для всій групи" . PHP_EOL . PHP_EOL . $text;
                $text = '';
            case 2:
                $this->conversation->update();
                ...
                $out_text = '/mailing Результат:' . PHP_EOL;
                $error_text = '';
                $result_text = '';

                foreach ($students as $index => $student) {
                    $data['chat_id'] = $student['user_id'];
                    $result = Request::sendMessage($data);
                    $student_user = DB::selectUserData($student['user_id']);
                    
                    if ($result->isOk()) {  
                        $result_text = $index . '/' . count($student) . PHP_EOL;
                    } else {
                        $error_text .= 'Помилка під час відправки повідомлення: ' . $student_user->first_name . ' ' . $student_user->second_name . PHP_EOL;
                    }
                }
